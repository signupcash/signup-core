...
function connectWalletToTxBridge(sessionId) {
  if (window.Worker) {
    ...
    myWorker.postMessage({ reqType: "connect", sessionId });
  } else {
      ...
      {status === "APPROVED" && (
        <>
          ...
          <Heading number={5} highlight>
            You can now go back to the application. Please keep this window open
            to allow {clientPayload.origin} to spend from your wallet up to{" "}
            {clientPayload.budget}
          </Heading>
          <p
            class={css`
              margin: 32px 16px;
