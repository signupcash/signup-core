...
export default function () {
  ...
  const [serializedPledgedCommitment, setSerializedPledgedCommitment] =
    useState("");
  const [buildingPledgeCommitment, setBuildingPledgeCommitment] =
    useState(false);
  ...
  useEffect(async () => {
    if (commitmentDetails && latestUtxos) {
      const totalAmountInSatoshis = await sats(
  ...
  async function parseCommitment(e) {
    ...
    try {
      const { outputs, donation, data, expires } = JSON.parse(
        Buffer.from(e.target.value, "base64").toString("utf16le")
      );
      const { amount } = donation;

      const isValidOutputs =
        outputs &&
        outputs.length &&
        !outputs.some(({ value, address }) => {
          return !value || value < DUST || !isCashAddress(address);
        });

      const isExpired =
        typeof expires !== "undefined" && moment().unix() > Number(expires);
      ...
      <Article>
        ...
        <form onSubmit={makeManualContribution} style="width:100%">
          ...
          {commitmentDetails ? (
            !serializedPledgedCommitment ? (
              <Button
                type="submit"
                primary
                disabled={!balanceIsEnough}
                customStyle={css`
                  margin-top: 16px;
                `}
              >
                {buildingPledgeCommitment
                  ? "Building..."
                  : balanceIsEnough
                  ? `Pledge ${totalAmountStr}${totalAmountInUSDStr}`
                  : `Pledge`}
              </Button>
            ) : (
              <Button
          ...
          <Button
            ...
            onClick={(e) => {
              setDetails(null);
              setUnserializedCommitment("");
              setSerializedPledgedCommitment("");
            }}
        ...
        {!utxoIsFetching &&
          (!frozenContributions || !frozenContributions.length) && (
            <Heading number={3}>No contributions</Heading>
          )}
        ...
        {!utxoIsFetching &&
          ...
          frozenContributions.map((utxo) => (
            ...
            >
              ...
              <Button
                disabled={!canRevokeContributions}
                onClick={() => handleContributionRevocation(utxo)}
                type="button"
                secondary
                customStyle={css`
                  background: none;
